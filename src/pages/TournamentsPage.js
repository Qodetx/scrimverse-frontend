...
const TournamentsPage = () => {
  ...
  const applyFilters = useCallback(() => {
    ...
    // Tournament size filter (max_participants)
    if (filters.tournamentSize !== 'all') {
      if (filters.tournamentSize.includes('+')) {
        const min = parseInt(filters.tournamentSize);
        filtered = filtered.filter((t) => (t.max_participants || 0) >= min);
      } else {
        const [min, max] = filters.tournamentSize.split('-').map((v) => parseInt(v));
        filtered = filtered.filter((t) => {
          const size = t.max_participants || 0;
    ...
    if (filters.planType !== 'all') {
      filtered = filtered.filter((t) => {
        if (filters.planType === 'featured') return t.is_featured === true;
        if (filters.planType === 'basic') return !t.is_featured;
        return true;
      });
    }
  ...
  useEffect(() => {
    applyFilters();
  }, [tournaments, searchQuery, filters, applyFilters]);
            ...
            <div className="filters-grid">
              ...
              <div className="filter-group">
                ...
                >
                  ...
                  <option value="0-50">0 - 50 Teams</option>
                  <option value="51-100">51 - 100 Teams</option>
                  <option value="101-200">101 - 200 Teams</option>
                  <option value="200+">200+ Teams</option>
                </select>
              </div>
              ...
              <div className="filter-group">
                ...
                >
                  ...
                  <option value="basic">Basic Tournaments</option>
                  <option value="featured">Featured Tournaments</option>
                </select>
              </div>
            </div>
          ...
          {loading ? (
            ...
            <div className="tournaments-grid">
              {filteredTournaments.map((tournament) => (
                <TournamentCard key={tournament.id} tournament={tournament} activeTab={activeTab} />
              ))}
            </div>
          )}
        </div>
